...
{      // В каком ряду активный юнит и есть ли ряд у активной стороны
      ...
      begin
        ...
        begin
          ...
          Exit;
			...
			U := ((GetInt('Slot7HP') > 0) or (GetInt('Slot8HP') > 0) or (GetInt('Slot9HP') > 0));
			if (U and (GetInt('SlotClick') >= 7) and (GetInt('SlotClick') <= 9)) 
			or (not U and (GetInt('SlotClick') >= 10) and (GetInt('SlotClick') <= 12)) then
			Run('Battles\HitUnit.pas');
		end;
	end;
	2: // Маги атакуют всю партию
	begin
		if (GetInt('SlotClick') > 6) then Run('Battles\HitUnits.pas');
	end;
	3: //  Исцелить воина
	begin
		if (GetInt('SlotClick') <= 6) then Run('Battles\HealUnit.pas');
	end;
	4: // Рейнжер атакует одного любого юнита из партии
	begin
		if (GetInt('SlotClick') > 6) then Run('Battles\HitUnit.pas');
	end;
	5: // Исцелить всю партию
	begin
		if (GetInt('SlotClick') <= 6) then Run('Battles\HealUnits.pas');
	end;
	...
	begin
		if (GetInt('SlotClick') > 6) then Run('Battles\SlepotaUnit.pas');
	end;
	9: // СЛЕПОТА на всех юнитов партии
	begin
		if (GetInt('SlotClick') > 6) then Run('Battles\SlepotaUnits.pas');
	end;

end else // Партия с правой стороны
case GetInt('Slot' + GetStr('ActiveCell') + 'Class') of
	1: // Воин атакует одного ближайшего юнита
	begin
		if (GetInt('SlotClick') <= 6) then
		begin
			...
			or (not U and (GetInt('SlotClick') >= 1) and (GetInt('SlotClick') <= 3)) then
			Run('Battles\HitUnit.pas');
		end;
	end;
	2: // Маги атакуют всю партию
